...
- **基本（免 AI）**：在「文獻與基本檢查」勾選「內文引用對應」，本機比對常見中英文作者／年份（含 a、b）與明示 `[編號]`。逐筆指出未找到書目、年份不同及多筆候選；書目對應相符不代表主張正確。可單獨執行，不傳送論文。
- **進階（需 AI Key）**：先確認基本書目，再於 AI 分析勾選「引用主張」。僅使用已查到且書目相符的 Crossref／OpenAlex 摘要，或在「補充引用來源證據」逐篇貼上摘要／全文相關段落和來源網址。不會自動下載或宣稱讀過全文。
- 逐筆列出 **Original 原句、對應文獻、來源範圍、逐字證據與處理建議**；結果分支持、部分支持、僅主題相關、矛盾及證據不足。疑點以紅色粗體顯示，不生成替換句、不改 Word。
- AI 引文無法在提供的來源逐字找到時，結果降為無法判定；無證據的項目不送 AI，照樣列入待查證報告。這項檢查不保證 AI 語意判讀正確，仍須作者核對全文。
- 網頁、HTML、PDF 均分成「引用對應 · 基本」與「主張支持 · 進階」；查閱時一次選一筆，PDF 包含全部結果。未辨識到引用不代表沒有引用；特殊格式、機構縮寫、註腳、圖片及跨段需人工確認。
- OpenAlex 摘要重建依據：[官方欄位文件](https://help.openalex.org/data/works/attributes/)。各資料來源不一定提供摘要；使用者補充的來源身分與完整性未經獨立驗證。
...
上傳 Word 後，開啟「文獻與基本檢查 · 免 AI」，確認參考文獻標題所在段落，再勾選資料庫與資料傳送同意。可選擇僅執行本機統計規則檢查，不傳送資料。

- 文獻逐篇列出 R 編號、原始書目、判定依據及來源。可篩選 OK、有疑點、待核對、查無紀錄、查詢未完成、人工確認。
- OK 僅表示完整題名、第一作者、年份及原文列出的 DOI 相符；不保證其他作者、卷期頁碼、樣式與全文主張正確。人工結果獨立標示，不冒充系統驗證。
- Crossref／OpenAlex／PubMed 查詢、人工查證、統計規則及 HTML／PDF 下載已移至基本功能。OpenAlex 選填金鑰是資料庫金鑰，不是 AI 金鑰。
- 報告先列每篇結論，再列對應的原文、核對欄位與實際來源；移除逐一重複未操作的人工資料庫說明。
- AI 僅處理需語意判讀的規範、引用樣式、文內引用與統計敘述，沿用已完成的基本檢查，不重複查詢資料庫。

## AI 進階分析與研究誠信

1. 匯入論文與規範，開啟「AI 進階分析」，選擇供應商並輸入自己的金鑰。
2. 選擇規範分析、參考文獻格式檢查、引用語意及統計敘述判讀。
...
- 首頁「文獻與基本檢查 · 免 AI」直接進入基本引用格式檢核，不需要 AI 金鑰，僅外部書目查詢需同意資料傳送。
- 基本／AI 檢查結果可按「開啟分區網頁報告」；總覽、文獻對照、內容與統計、排版建議、AI 摘要、外部紀錄與範圍責任依本次結果顯示。選擇單篇／單項閱讀，不需要翻找整份長報告。
- 「論文內容 vs. 建議修改後／處理方式」並排呈現。文獻建議是需人工確認的草稿，缺漏用佔位文字指出，不能當成已驗證書目；不写回 Word。統計只給核對方式，不提供替換值。
- 「測試此模型」需明確點擊與費用同意，會產生少量 API 用量，但不傳送論文。只有真正通過結構化輸出或分析的模型才標示已驗證。
- localStorage 僅保存供應商／金鑰雜湊識別、模型 ID、狀態與日期，不保存 API Key。重新輸入相同金鑰可取回已驗證清單。403 的明確模型權限錯誤、404 與不支援的模型設定會排除該模型；401、429、網路錯誤不判定模型不可用。可手動清除紀錄重新檢核。
- Gemini 即時清單移除不支援 generateContent 或已確認無法存取的模型；未實測模型清楚標示待實測。已驗證並不保證未來額度或權限不變。
